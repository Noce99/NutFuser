...
class TransfuserBackbone(nn.Module):
  ...
  def __init__(self, config):
    ...
    else: # <---------------- config.lidar_architecture = regnety_032
  ...
  def forward(self, image, lidar):
    ...
    if self.config.normalize_imagenet: # <---------- TRUE
    ...
    # BOTH THE IMAGE AND LIDAR ENCODERS are a regnety_032 (https://paperswithcode.com/lib/timm/regnety#)
    ...
    if len(self.lidar_encoder.return_layers) > 4: # <------------- TRUE
    ...
    for i in range(4):
      ...
      image_features, lidar_features = self.fuse_features(image_features, lidar_features, i) # TO BE EXPLORED!!!!!!!!!!!!!!
      # print(f"@"*10)
    # print(f"-"*64)
    if self.config.detect_boxes or self.config.use_bev_semantic: # <------------------------ True or True
  ...
  def forward_layer_block(self, layers, return_layers, features):
    ...
    """
    # return_layers = {'stem': '0', 's1': '1', 's2': '2', 's3': '3', 's4': '4'}
  ...
  def fuse_features(self, image_features, lidar_features, layer_idx):
    ...
    # print(f"lidar_embd_layer = [{lidar_embd_layer.shape}]")
    ...
    image_features_layer = F.interpolate(image_features_layer,
                                         ...
                                         mode='bilinear',
                                         align_corners=False)
    if self.lidar_video: # NOT ENTERING False
...
class GPT(nn.Module):
  ...
  def __init__(self, n_embd, config, lidar_video, lidar_time_frames):
    ...
    self.seq_len = 1
    self.lidar_video = lidar_video # False
    ...
    self.blocks = nn.Sequential(*[
        ...
        for layer in range(config.n_layer) # n_layer = 2
    ])
...
class Block(nn.Module):
  ...
  def __init__(self, n_embd, n_head, block_exp, attn_pdrop, resid_pdrop):
    super().__init__()
    """
    n_embd is one of the following: [72, 216, 576, 1512]
    block_exp is 4
    """
