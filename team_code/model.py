...
class LidarCenterNet(nn.Module):
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command):
    ...
    my_lidar_in_np[123:131, 120:134, 0] = 255
    my_lidar_in_np[123:131, 120:134, 1] = 0
    my_lidar_in_np[123:131, 120:134, 2] = 0
    ...
    def check_output(output_name, output):
        if output is not None:
            if torch.is_tensor(output):
                print(f"Tensor {output_name} = {output.shape} [{output.min()}; {output.max()}]")
            else:
                for i, small_output in enumerate(output):
                    if torch.is_tensor(small_output):
                        print(f"{output_name}_{i} = {small_output.shape} [{small_output.min()}; {small_output.max()}]")
        else:
            print(f"{output_name} is None!")
    
    check_output("pred_wp", pred_wp)
    check_output("pred_target_speed", pred_target_speed)
    check_output("pred_checkpoint", pred_checkpoint)
    for i in range(pred_checkpoint.shape[1]):
        my_lidar_in_np = cv2.circle(my_lidar_in_np, (int(127+pred_checkpoint[0, i, 0]/0.125), int(127+pred_checkpoint[0, i, 1]/0.125)), 4, (0, 0, 255), thickness=1) 
    check_output("pred_semantic", pred_semantic)
    check_output("pred_bev_semantic", pred_bev_semantic)
    check_output("pred_depth", pred_depth)
    check_output("pred_bounding_box", pred_bounding_box)
    check_output("attention_weights", attention_weights)
    check_output("pred_wp_1", pred_wp_1)
    check_output("selected_path", selected_path)
    """

    # SHOWING IMAGES by Noce

    if pred_depth.shape[0] == 1:
      my_pred_depth_in_np = (pred_depth*255).cpu().numpy().astype(np.uint8) # (1, 256, 1024)
      my_pred_depth_in_np = np.moveaxis(my_pred_depth_in_np, 0, -1)
      # print(f"DEPTH min = {my_pred_depth_in_np.min()}, max = {my_pred_depth_in_np.max()}")
      #cv2.imshow('Depth', my_pred_depth_in_np)
      #cv2.waitKey(1)
      ...
      #cv2.imshow('Bev Semantic', bev_semantic)
      #cv2.waitKey(1)
      ...
      #cv2.imshow('Camera', cv2.resize(my_imag_in_np,dsize=None,fx=1,fy=1))
      #cv2.waitKey(1)
      #cv2.imshow('Lidar', cv2.resize(my_lidar_in_np,dsize=None,fx=2,fy=2))
      #cv2.waitKey(1)
      ...
      output_image[256+20+256+20:256+20+256+20+256, 1024+20:1024+20+256] = np.rot90(my_lidar_in_np)
